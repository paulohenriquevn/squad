...
- Before PLAN starts: a discovery opportunity exists OR the user explicitly confirms no measurement is needed (deterministic; pre-recorded via `--no-discover`). Enforced by `assess_confidence.py`, which reads the opportunity from disk; a phase entered out of order is caught after the fact by `check_phase_drift.py` against `cycle-events.jsonl`.
...
- Before IMPLEMENT starts: plan-confidence verdict ≥ SHIPPABLE_WITH_CAVEATS — emitted by `run_structural.py` and confronted with the declared chain by `check_phase_drift.py`.
- Before CODE-QUALITY starts: implementation emitted `IMPLEMENTATION_COMPLETE` — recorded by `cycle_events.py` and confronted with the declared chain by `check_phase_drift.py`.
- Before REVIEW starts: code-quality verdict ∈ {`PASS`, `PASS_WITH_CAVEATS`} — emitted by `run_code_quality.py`, banded by `rules/verdict-bands.txt`, and confronted with the chain by `check_phase_drift.py`.
