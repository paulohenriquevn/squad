...
| CVE CRITICAL/HIGH em dependência declarada → score ≤ 49 (INVALID) | `check_deps_audit.py` — lê o VEREDITO que `/deps-audit` deixou em `knowledge-base/audits/{slug}-deps-audit-*.md`. Não procura CVE: quem faz isso são os scanners daquela skill. Stable id: `deps_audit_insecure`. **Extensão registrada 2026-08-26**, com a razão: `cycle-plan.md § Phase contracts` listava este gate entre os hard gates da fase e declarava, na linha seguinte, que era "the one gate in this cycle nothing mechanizes" — ele valia só quando alguém lembrava de rodar a skill e de honrar o resultado. O § "When this rule may change" abaixo pede ADR para estender o gate; os arquivos sob `knowledge-base/adrs/` são gitignored e não alcançam quem clona, então o registro vive aqui, no arquivo que viaja. |
| Dependência declarada sem auditoria nenhuma → score ≤ 89 | `check_deps_audit.py`. Stable id: `soft_floor_deps_audit_missing`. Soft floor, não hard cap: afirmar CVE seria uma afirmação sobre a dependência que ninguém fez. O que se afirma é sobre o PROCESSO — a verificação não aconteceu. Ausência de auditoria nunca vira "sem CVE". |
