...
| Baseline Context section missing OR placeholder-laden → score ≤ 89 (≤ 89; a sunset of 2026-09-07 promising ≤ 70 stood here, passed, and nothing noticed — the promotion is a decision, not a date) | M4 v1.0 — `check_baseline_context.py`. The section is the "deep review of current state" — file table + LoC + git sha + callers + glossary + architecture boundaries. Junior implementer should not need to spelunk the repo. Stable id: `soft_floor_baseline_context_incomplete`. See § SOTA upgrade below. |
| Drawbacks & Risks section missing OR < 2 entries → score ≤ 89 (≤ 89; a sunset of 2026-09-07 promising ≤ 70 stood here, passed, and nothing noticed — the promotion is a decision, not a date) | M4 v1.0 — `check_drawbacks_section.py`. RFC tradition — no non-trivial plan is risk-free. Stable id: `soft_floor_drawbacks_section_insufficient`. |
| Unresolved Questions section missing AND no explicit "(none)" marker → score ≤ 89 (≤ 89; a sunset of 2026-09-07 promising ≤ 70 stood here, passed, and nothing noticed — the promotion is a decision, not a date) | M4 v1.0 — `check_drawbacks_section.py` (covers both Drawbacks & Unresolved). Stable id: `soft_floor_unresolved_questions_section_missing`. |
