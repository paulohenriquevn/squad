...
| CRITICAL/HIGH CVE in a declared dependency → score ≤ 49 (INVALID) | `check_deps_audit.py` — reads the VERDICT `/deps-audit` left in `knowledge-base/audits/{slug}-deps-audit-*.md`. It does not look for CVEs: that skill's scanners do. Stable id: `deps_audit_insecure`. **Extension recorded 2026-08-26**, with the reason: `cycle-plan.md § Phase contracts` listed this gate among the phase's hard gates and declared, on the next line, that it was "the one gate in this cycle nothing mechanizes" — it held only when someone remembered to run the skill and to honour the result. The § "When this rule may change" below requires an ADR to extend the gate; the files under `knowledge-base/adrs/` are gitignored and do not reach whoever clones, so the record lives here, in the file that travels. |
