...
Validate whether a project is **on the correct trajectory** using quantitative evidence — benchmarks, complexity metrics, architecture fitness functions, scalability projections, and reference comparisons. Unlike `/code-quality` (structural correctness) or `/review` (merge readiness), `/trajectory-review` answers: "Given the evidence, will this architecture meet its goals at target scale?"

The trajectory-review follows the scientific method: extract hypotheses from project documentation, formulate measurable predictions, run experiments (benchmarks + profiling), and conclude with evidence-backed verdicts. Every finding cites numbers, methodology, and reproduction commands.
...
When enabled, `/trajectory-review` is the **last cycle** before the roadmap super-loop selects the next milestone. Its verdict drives the shape of the next iteration — see § Verdicts below for the full token → feedback-action mapping.

## Pre-conditions

- `/release` has completed — trajectory-review runs on **released code**, not in-progress work. This ensures the measured state is the state that shipped.
- Project has `rules/trajectory-review-config.txt` with `enabled = true`. Without this, `/trajectory-review` exits with INFO "trajectory-review not enabled for this project".
...
- Working tree is clean (no uncommitted changes — trajectory-review reads a stable state).
- Compilable/runnable state (tests pass, project builds).

Do NOT trigger when:

- Project has no `trajectory-review-config.txt` or `enabled = false` — the cycle is opt-in by design.
...
- `/release` has NOT completed — trajectory-review measures released state, not mid-development state.
...
/trajectory-review [plan-slug]
     ...
     ↓ load trajectory-review-config.txt + trajectory-review-golden-rule.md
     ↓ Phase 1: extract hypotheses from CLAUDE.md, plans, ADRs, README
     ↓ Phase 2: run trajectory-review modules A1-A6 (per profile weights)
...
| **A4 — Memory** | Per-object overhead, allocation count, unsafe ratio | Measured via profiler or sizeof trajectory-review, not theoretical |
...
| Gate | Trigger | Source |
|---|---|---|
> **None of the six caps below is computed.** Measured 2026-08-27: `skills/trajectory-review/` ships a `SKILL.md` and no scripts at all, so the verdict here is named by the agent that wrote the report rather than derived from it — the shape `/acceptance` exists to refuse (*"the verdict is computed, never asserted"*). The sibling kit closed this with a `compute_analysis_verdict.py`; this one has not. Every row is therefore _(not mechanized: no script in this repository reads the golden rule and emits a verdict)_ until that script lands.

| Golden rule missing | `trajectory-review-golden-rule.md` not found or unparseable | § 5.1 of golden rule |
| Config not enabled | `trajectory-review-config.txt` missing or `enabled ≠ true` | § 5.2 of golden rule |
...
- **Running before release** — trajectory-review measures released state. Mid-development measurements are noise, not signal.
...
- **Skipping hypothesis extraction** — running benchmarks without hypotheses is just benchmarking, not trajectory-review. The hypothesis-driven approach is what makes this PhD-level.
...
- Golden rule: `rules/trajectory-review-golden-rule.md`
- Config: `rules/trajectory-review-config.txt`
- Skill: `skills/trajectory-review/SKILL.md`
- Languages (reused): `rules/code-quality-languages.txt`
- Upstream: `rules/cycle-release.md` (trajectory-review runs after release completes)
- Feedback targets: `rules/cycle-discover.md` (FUNDAMENTAL_RETHINK), `rules/cycle-plan.md` (COURSE_CORRECTION / risk injection)
- Macro super-loop: `rules/cycle-maintenance.md` (trajectory-review verdict shapes next milestone)
