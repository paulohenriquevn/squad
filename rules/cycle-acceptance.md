...
- **No flip without a green verdict.** `[x]` claims a user-visible promise was met; only `ACCEPTED` / `ACCEPTED_WITH_CAVEATS` may flip it — _(not mechanized: a retired session-binding skill used to refuse a session release on any other token, which caught a wrong flip after the fact. It was cut on 2026-08-31, and the flip script has never read the verdict itself. The gate is now honoured by discipline at both ends. Stated rather than quietly dropped: the cut removed a check, and pretending otherwise is the exact failure this cycle exists to prevent)_
...
- **The target is the released artifact.** Validating a local build, a staging clone, or a mock reproduces exactly the blind spot this cycle exists to remove. The project declares how its published delivery is reached in `rules/acceptance-target.txt` — _(not mechanized: a retired skill refused to arm a goal until that file was filled, which forced the declaration to exist before a run. Nothing enforces it now, and whether what was exercised IS the released artifact was always read from the evidence by a human)_
