...
1. Resolve the active plan:
   ...
   - Then newest `.squad/records/plans/*-plan.md` by mtime
2. Read the resolved plan file.
3. Derive a goal condition from the plan's content:
   - **Default condition:** "all Objective checkboxes in `.squad/records/plans/{slug}-plan.md` are checked AND the named metric in the Goal section is observable AND the Global DoD section's checkboxes all check"
...
- If `.squad/records/plans/{slug}-plan.md` does not exist, this command refuses with "no plan found; run /plan-write first".
