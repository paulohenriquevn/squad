...
- **Idea-to-merge in one chain.** Eight composable cycles (Roadmap → Discover → Plan → Implement → Code-Quality → Review → Release → Acceptance) that you enter at the lightest point matching your task.
- **`[x]` means validated, not merely shipped.** The roadmap checkbox flips only after `cycle-acceptance` drives the *released* delivery through the milestone's own Definition of done and records evidence per criterion.
...
```
            ...
            │  → verdict: RELEASED                     │              │
            └──────────────────┬───────────────────────┘              │
                               │ RELEASED (tag + GH release live)     │
                               ▼                                      │
            ┌──────────────────────────────────────────┐              │
            │              ACCEPTANCE                  │              │
            │  drive the RELEASED delivery as its user │              │
            │  (chrome-devtools / cua-driver / CLI /   │              │
            │   API) against the milestone DoD         │              │
            │  evidence per criterion — no evidence,   │              │
            │  no pass; verdict computed, not asserted │──────────────┘
            │  ACCEPTED → ROADMAP.md M<N> [ ]→[x]      │   loop back to ROADMAP
            │  → verdict: MILESTONE_RELEASED           │   until ROADMAP_COMPLETE
            └──────────────────────────────────────────┘
...
├── ROADMAP.md                     ← macro milestones (M0..M8) — written by /roadmap-init, checkbox flipped by cycle-acceptance
...
│   ├── acceptance/                ← cycle-acceptance: end-user validation of the release + checkbox flip
...
│   ├── cycle-acceptance.md        ← acceptance cycle contract (validate the release; gate the roadmap flip)
...
| `cycle-acceptance` | Validating the RELEASED delivery as its end user meets it — the gate on the milestone `[x]` | `/release` did not reach `RELEASED`; the delivery is not reachable |
| `/roadmap-review` (auxiliary) | Reviewing a ROADMAP.md for edge cases, inconsistencies, clarity and cohesion before the team executes it | There is no `ROADMAP.md` yet — run `/roadmap-init` |
