...
| `check_review_binding.py` | Refuses to promote work whose review is about a different commit. A verdict said the work was examined and never said WHICH work, so a commit landing after consolidation travelled on an approval that never saw it — the approval was bound to a NAME, not a CONTENT. Reports what moved and refuses only on overlap with the files the review examined: a gate that blocked on any movement would be bypassed within a week. A record listing no files widens to every move rather than assuming harmlessness |
