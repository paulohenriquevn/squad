...
| `check_data_root.py` | Reports a project still holding data outside `<project>/.squad/`. Reports and never moves: a migration run inside a repository the kit does not own is the kit writing to somebody else's project. Failing states, loudest first: `COMMITTABLE` (the study zone is not gitignored), `INSIDE_KIT` (data written into the installed kit), `NESTED` (a write root inside the write root), `SPLIT` (both roots hold data and the old copy is unreachable while still looking current), `SHARED` (kit leaves mixed into a plugin's wiki bundle), `UNMIGRATED`. `FOREIGN` — another producer's bundle in `wiki/` — is reported and never fails |
...
| `check_plugin_freshness.py` | Does the plugin a REVIEW would audit WITH match the repository it was built from? Claude Code installs into a CACHE and this kit resolves plugins by `installPath`, so an audit runs that snapshot rather than the source. Measured 2026-09-22 by the session maintaining them: 17 of 18 installed plugins were behind their repositories, and the contract under test did not exist in the tree that ran — the repo was right, the reader was right, what executed was neither. A premise asked once before the first item, like `check_merge_autonomy.py`. Scope is `rules/review-auditors.txt`, not the machine. `unverifiable` (source not local, no `gitCommitSha`, plugin absent) is reported apart from `aligned` and never as a failure |
| `check_verification_freshness.py` | When did this tree last verify itself, and does the answer still apply? `run_slice_tests.sh` printed its verdict and exited, so the only way to answer *is it green?* was to run it again — measured 2026-09-22, one session ran it four times in a day and two of the four answered about a tree that had moved. The runner now writes `.squad/records/verification/last-run.json` and this reads it, in milliseconds rather than fifteen minutes, which is the property that made the question go unasked. Six states: `verified`, `stale` (the record names a commit this tree moved past), `failing`, `unattributable` (the runner itself said the tree moved mid-run), `never` and `unreadable`. `never` is NOT `failing` — a fresh clone has not verified itself and is not broken, and collapsing them would fire on every checkout. Advises inside `promote_to_develop.py`; refuses nothing |
...
| `sprint.py` | Opens, refills, inspects and closes the active block of work — the declared goal a queue ordered by age does not have. It adds exactly three things the kit lacked: a GOAL, a CLOSE that records what each admitted item came to, and an IDENTITY. It builds no WIP limit, no replenishment and no blocked-item disposition, because `pipeline_orchestrator.py`, `fleet_router.py` and `halt_disposition.py` already are those, and a fourth answer to one question is this repository's most frequent defect. Refuses a sprint with no goal, one admitting nothing, a signature that is not `human/<name>`, and a close over an item that can still move |
