...
| `check_panel_capability.py` | Can a review panel be formed from `rules/review-panel.txt` at all? Three reviewers, at least one from a recognised family outside the kit's own, every non-builtin one on PATH. Asked at intake because otherwise every item is measured, planned, and returned at a panel that was never formable |
