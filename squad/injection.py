...
DESIGNED AND MEASURED IN A CONSUMER, adopted into the kit on 2026-09-22 with the
sponsor's decision. It was written inside an installed `.claude/`, where the next
`install.sh --force` would have erased it — the shape `boundaries.violation` names
and #164 filed. The numbers below are that session's, taken on its own tree.
