...
| "Check the released thing actually works for its user" | `cycle-acceptance` | `/acceptance M<N>` |
| "Review the roadmap itself before committing to it" | auxiliary | `/roadmap-review` |
...
```
git switch workspace   # all work is born here (Unbreakable Rule 4 / git-safety.md § 1)
/implement {plan-slug}
...
/release [patch|minor|major]
  ...
  → commits "chore(release): {version}" on workspace
...
| `cycle-acceptance.md` | acceptance | Validating the released delivery end-to-end after `/release` returned `RELEASED` |
