...
| `mechanisms/gates/check_evidence_freshness.py` | `/backlog-review` | reports each open item's evidence AGE and fails on a cited path that resolves nowhere. Age never fails: an old measurement of something nobody touched is still true. Run it from the PROJECT root — the kit's own repository has no versioned `BACKLOG.md`, so here it can only report that nothing was swept |
...
| `illegal_subject` | major | `subject:` outside `product \| kit`. A typo that is ignored is a routing decision the author believes they declared |
| `subject_belongs_to_the_kit` | blocker | A consumer's registry holding an item that changes the installed kit. It cannot close there: the kit under `.claude/` is not versioned, so a fix written there protects one machine and the next install overwrites it. File it in the kit's own tracker |
| `subject_may_belong_to_the_kit` | minor | Same, suspected from evidence that names kit paths and no product ones. **ADVISORY and never a failure** — the detector is a path heuristic that was arguable on three of thirteen when measured. Declaring `subject:` stops the guessing |
