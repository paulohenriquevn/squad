...
arguments, `argument-hint`. One skill does **not**: `skill-creator` is vendored from
Anthropic and kept byte-close to upstream, so re-syncing it is a copy rather than a
merge. `python3 scripts/validate_skill_frontmatter.py` accepts that on
