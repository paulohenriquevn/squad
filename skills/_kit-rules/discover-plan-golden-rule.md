...
2. **Fabricated source citation** — at least one `study-material/{project}` cited in the plan does not exist when checked via `Path.exists()`.
...
| All `study-material/{...}` paths resolve | `check_measurement_targets.py` |
