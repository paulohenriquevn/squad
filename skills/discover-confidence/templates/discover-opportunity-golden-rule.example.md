...
A opportunity that is missing a coverage corner is a opportunity that did NOT do the deep-research it claimed. A opportunity with fabricated citations is unsafe to use as a design source — recipients will trust the cited paths and build on top of references that don't exist.

The lesson (mirrors plan-confidence's): **tests passing ≠ system works; coverage table green ≠ research complete.** A opportunity with one empty corner or one fabricated citation will produce wrong decisions downstream, even if 90% of other checks are green.
...
3. Bump `discover-opportunity-thresholds.txt` reference to the new ADR.
4. Add log entry with date and reason.

## Related

- Skill: `.claude/skills/discover-confidence/SKILL.md`
- Thresholds: `.claude/rules/discover-opportunity-thresholds.txt`
- Rubric: `.claude/skills/discover-confidence/templates/rubric-opportunity.md`
