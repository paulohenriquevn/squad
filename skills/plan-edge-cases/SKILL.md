---
name: plan-edge-cases
version: 0.1.0
requires: [plan-write]
description: Analyzes an implementation plan and identifies unforeseen edge cases. Pragmatic — flags real risks without complicating the design. Use after /plan-write or when reviewing any plan in .squad/records/plans/.
user-invocable: true
...
- `$ARGUMENTS` = plan slug (resolved against `.squad/records/plans/{slug}-plan.md`) or a full path
- No argument = analyze the most recent plan in `.squad/records/plans/`
...
```!
# Locate the plan
ls .squad/records/plans/*${ARGUMENTS}* 2>/dev/null || ls -t .squad/records/plans/*.md | head -5
```
...
```
.squad/records/reviews/{plan-slug}-edge-cases-{YYYY-MM-DD}.md
```
...
- Runs AFTER `/plan-write` or whenever someone asks for a review of a plan in `.squad/records/plans/`
