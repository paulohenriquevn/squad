...
def test_renormalize_weights_m2_proportions_correct() -> None:
    weights = renormalize_weights(["completeness", "structural_risk"])
    # SOTA: completeness=0.30, structural_risk=0.20. Sum 0.50. completeness/0.50 = 0.6.
...
def test_run_structural_good_plan_passes() -> None:
    ...
    assert report.final_score_after_caps >= 70, (
        ...
        f"completeness={report.completeness_score}, structural_risk={report.structural_risk_score}"
    )
...
def test_run_structural_evidence_inactive_in_m2_weights() -> None:
    ...
    reasons for visibility — they just do not contribute to the score. After the
    SOTA upgrade the good-plan fixture cites real rule files (Baseline Context,
    Prior Art, Drawbacks references), so positive evidence reasons may be present
