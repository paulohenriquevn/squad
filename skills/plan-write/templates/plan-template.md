...
This file is the canonical template referenced from `plan-write/SKILL.md § Plan Template`. To produce a new plan, copy everything inside the `<plan-template>` block below into `records/plans/{slug}-plan.md` and replace placeholders.
...
- **Internal blueprints** (`records/discoveries/blueprints/`) — cite `Blueprint §"Q1"` style; section must exist in a real blueprint file.
...
- **Reference projects** (`records/references/{project}/`) — cite `file:line` evidence for any technique borrowed.
