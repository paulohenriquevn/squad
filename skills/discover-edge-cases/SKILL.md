...
- Slug (`web-console-trace-latency`): `Glob` under `.squad/records/discoveries/plans/*{slug}*.md`
- A `.md` path: use directly
- No hint: most recent file under `.squad/records/discoveries/plans/` by mtime
...
```bash
ls .squad/records/discoveries/plans/*${ARGUMENTS}* 2>/dev/null || ls -t .squad/records/discoveries/plans/*.md | head -5
```
...
Date: YYYY-MM-DD
Plan analyzed: .squad/records/discoveries/plans/{slug}-plan.md
