...
Detects all manifests in repo root (excluding `node_modules/`, `.venv/`, `__pycache__/`, `study-material/`, `dist/`, `build/`, `target/`), runs the matching auditors, prints a report to stdout. No plan binding; advisory only.
...
Reads `.squad/records/plans/{slug}-plan.md` (or `.squad/records/discoveries/plans/{slug}-plan.md`), parses its `## Dependencies` section, cross-references every declared dep against:
...
In **Mode 2**: write to `.squad/records/audits/{slug}-deps-audit-{date}.md` AND print summary to stdout. The persistent file is the audit trail consumed by `/plan-confidence`.
...
12. **NEVER use `study-material/` clones as part of the audit surface** — read-only zone; their deps are not OUR responsibility.
...
|---|---|
| Report at `.squad/records/audits/{slug}-deps-audit-{date}.md` | Delete file; no further state to revert. |
