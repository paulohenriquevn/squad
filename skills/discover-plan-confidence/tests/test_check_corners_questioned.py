"""Tests for check_corners_questioned.py (plan variant) — question-to-corner mapping."""
from __future__ import annotations

from pathlib import Path

from check_corners_questioned import (
    CORNERS,
    _has_defer_corner_marker,
    check_corners_questioned,
)


def test_good_plan_covers_all_corners(good_measurement_plan: Path) -> None:
    report = check_corners_questioned(good_measurement_plan)
...
def test_synthetic_plan_covers_all_corners(synthetic_measurement_plan: Path) -> None:
    report = check_corners_questioned(synthetic_measurement_plan)
    assert report["corners_populated"] == 4
    assert report["empty_corners"] == []


def test_missing_corner_detected(missing_corner_measurement_plan: Path) -> None:
    report = check_corners_questioned(missing_corner_measurement_plan)
...
def test_plan_without_questions_section_covers_nothing(tmp_path: Path) -> None:
    ...
    report = check_corners_questioned(plan)
