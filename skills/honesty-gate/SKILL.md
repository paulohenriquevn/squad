...
Applies the project's **honesty-gate golden rule** (`rules/honesty-gate-golden-rule.md`) to the manifest at `knowledge-base/honesty-gate/manifest.md` and evidence files in `knowledge-base/honesty-gate/evidence/`. Emits a verdict on whether the project may legitimately claim `production-ready` / `production-grade` / `v1.0 ready`.
...
2. **A manifest** at `knowledge-base/honesty-gate/manifest.md` — declares the project's anchor scenario slug, current `Status`, and target dates.
3. **An evidence directory** at `knowledge-base/honesty-gate/evidence/` — append-only log of honesty-gate runs. Each file carries frontmatter with `scenario:`, `date:`, and a structured summary of what was exercised.
...
Check that `rules/honesty-gate-golden-rule.md`, `knowledge-base/honesty-gate/manifest.md`, and `knowledge-base/honesty-gate/evidence/` all exist. Missing any → emit the corresponding flag + verdict `EVIDENCE_INSUFFICIENT`. Stop.
...
| 3 | `glob` of `knowledge-base/honesty-gate/evidence/*.md` returns ≥ 1 file whose frontmatter `scenario:` matches the anchor slug | `no_anchor_evidence` |
...
Interactive: asks for scenario slug, date (defaults to today), and a summary template. Writes `knowledge-base/honesty-gate/evidence/{scenario}-{YYYY-MM-DD-HHMMSS}.md` with the required frontmatter.
...
- `audit` and `log-evidence` modes write to `knowledge-base/reviews/honesty-gate-{YYYY-MM-DD}.md`.
