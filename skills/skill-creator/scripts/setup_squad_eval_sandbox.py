...
def _write_plan(dest: Path, slug: str) -> None:
    ...
    plan.write_text(
        ...
        "## Falsification\n\n"
        "Se a contagem de queries for CONSTANTE em relação à contagem de spans, a hipótese\n"
        "está morta e B-014 é encerrado com esse resultado como `kill_reason`.\n\n"
        "Escrito como constância, não como 'uma query só': o DoD do item pede número\n"
        "independente da contagem de spans, e a forma em lote emite DUAS queries — ela\n"
        "satisfaz o DoD e falharia um critério escrito como 'uma só'. Um eval pegou essa\n"
        "contradição entre critério e DoD.\n\n"
        ...
        "|---|---|---|---|---|---|\n"
        "| Q1 | Quantas queries `listTraces` emite por request? | evidence | Read | `theo-lens/src/api/traces.ts:31-57` | contagem + file:line |\n"
        "| Q2 | O que mais consome esse handler? | blast_radius | Grep | `theo-lens/src/` | lista de chamadores |\n"
        "| Q3 | Como saberemos que o fix funcionou? | verification | Read | `theo-lens/src/api/traces.ts:65-89` | critério pass/fail |\n""| Q4 | `listTraces` tem chamador, ou é código morto? | evidence | Grep | `theo-lens/src/` | lista de chamadores, ou ausência declarada |\n\n"
