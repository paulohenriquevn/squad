...
"""Confront `rules/squad-map.md` with the directory it claims to describe.
    ...
    python3 scripts/check_squad_map.py [--root .] [--json]
...
THIS map claims:
  ...
  4. hooks         — every `hooks/*.sh` is named, and no phantom        (both)
...
catching it needs a name shape unambiguous enough to extract from prose. `*.sh`
...
def _hooks(root: Path) -> set[str]:
    return {p.name for p in (root / "hooks").glob("*.sh")}
...
HOOK_MENTION_RE = re.compile(r"\b([a-z][a-z0-9-]*\.sh)\b")
...
def check(root: Path) -> list[dict]:
    ...
    # 4. hooks — both directions: `*.sh` is an unambiguous name shape.
