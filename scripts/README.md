...
|---|---|---|
| `attest_plan.sh` | `commands/plan-attest.md` | Computes SHA256 of a plan file and writes it to `.attestations/{slug}.sha256` atomically (temp file + rename). Supports `--all` (attest every plan), `--verify` (read-only check). Dual-mode: standalone or `.claude/`-wrapped. |
...
| `session_catchup.py` | Recovery after `hooks/precompact-preserve.sh` | Rebuilds context from snapshots under `.compaction-snapshots/` + recent progress entries. Useful when the foreground session has been compacted and needs to resume mid-plan. |
| `verify_ecosystem.py` | Manual / CI | End-to-end smoke test that exercises the pipeline against a synthetic plan. Used to validate that hook + skill wiring still works after refactors. |

## When to run them

- **`attest_plan.sh`** — never run directly; let `/plan-attest` invoke it.
...
- **`session_catchup.py`** — run after PreCompact has fired and you want to brief a fresh session on the active plan + recent progress.
- **`verify_ecosystem.py`** — run when modifying hooks, settings, or the cycle rules themselves.
